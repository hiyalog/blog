...
1. 작성한 코드가 홀수 일때는 문제가 없었지만 짝수일때는 중앙값 계산이 문제가 되어 
   홀수 일때와 짝수일때 비교하여 다르게 작성해주어야 한다.  
2. 등차수열 공식으로 풀면 간단하게 풀수도 있다.  
3. 1항을 구해서 1항부터 항의 개수 num 까지 반복하면서 answer 배열에 넣고 리턴하면 된다.   
<br>
[-] 등차수열 풀이 방법  
설명은 길지만 내용은 간단한다.  
(<b>a1</b>=1항, <b>an</b>=마지막항, <b>total</b>=등차수열의 합, <b>num</b>=항의 개수)  
등차수열의 합은 1 항과 마지막 항을 더한 뒤 항의 개수를 곱하고 2로 나눈 값이다.  
이것을 식으로 적어보면  
...
|2|등차수열의 합(total)에 2를 곱하고 항의 개수(num)으로 나누면 1항과 마지막항을 더한값(a1+an)이 된다.  
   수식으로 적어보면  
   <b>sum = (total*2) / num</b>|
|3|sum 은 1항과 마지막항을 더한값이다.  
   수식으로 적어보면  
   <b>sum = a1 + an</b>|
|4|공차가 1 이기때문에 마지막항(an)은 (1항+(항의개수-1))과 같다.  
   수식으로 적어보면  
   <b>an = a1 + (num-1)</b>|
|5|<b>sum = a1 + an</b> 은 <b>sum = a1 + (a1+(num-1))</b> 과 같다.|  
<br>
수식을 정리해 보면  
[1] sum = a1 + an  
[2] sum = a1 + (a1 + (num-1))  
...
[5] 2a1 = sum - (num-1)  
[6] a1 = (sum - (num-1)) / 2  
[7] sum은 1항과 마지막항을 더한값으로  
   ((total*2) / num) 식으로 구할 수 있다.  
[8] <b>a1 = (((total*2)/num) - (num-1)) / 2</b>  
[9] 이렇게 하면 a1 즉, 1항값을 구할수 있다.  
