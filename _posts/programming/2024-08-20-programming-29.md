...
4. st_result에 저장된 정답갯수중 최대값을 구한다.( max()함수 사용 )  
5. st_result에 저장된 정답갯수와 최대값을 비교하여 같으면 answer 배열에 저장한다.  
6. answer배열을 오름차순으로 정렬하여 리턴한다.  
7. [주의할점] 매개변수 answers와 지역변수 answer가 이름이 비슷하다.  
...
def solution(answers):
    ...
    for i in st:
        ...
        st_result[i] = 0 # 정답갯수를 저장할 딕셔너리 자료형 변수
